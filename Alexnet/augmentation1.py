...
def main():
    ...
    # Create transforms
    train_transform_1 = transforms.Compose([transforms.Resize((240, 240)),
                                        transforms.RandomCrop([224, 224]),
                                        transforms.RandomHorizontalFlip(p=0.5),
                                        transforms.ToTensor(), 
                                        transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])])
    test_transform_1 = transforms.Compose([transforms.Resize((240, 240)),
                                       transforms.RandomCrop([224, 224]),
                                       transforms.RandomHorizontalFlip(p=0.5),
                                       transforms.ToTensor(), 
                                       transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])])
    ...
    train_images_1 = ImageFolder(root= "./data/train", transform=train_transform_1)
    test_images_1 = ImageFolder(root= "./data/test", transform=test_transform_1)

    # Create data loader
    trainDataLoader_1 = torch.utils.data.DataLoader(train_images_1,batch_size=128,num_workers=2, shuffle=True)
    testDataLoader_1 = torch.utils.data.DataLoader(test_images_1,batch_size=100,num_workers=2, shuffle=False)
    ...
    for epoch in range(50):
        ...
        train_loss, train_accuracy = train(trainDataLoader_1, optimizer, Loss, device, model)                             
        test_loss, test_accuracy = test(testDataLoader_1, Loss, device, model)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("./model_plots/aug1_%s_loss.png" % args.optimizer)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("./model_plots/aug1_%s_accuracy.png" % args.optimizer)
