...
def main():
    ...
    # Create transforms
    train_transform_3 = transforms.Compose([transforms.Resize((224, 224)),
                                        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.3),
                                        transforms.Pad([8, 8, 8, 8], fill=0, padding_mode='constant'),
                                        transforms.ToTensor(),
                                        transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])])
    test_transform_3 = transforms.Compose([transforms.Resize((224, 224)),
                                       transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.3),
                                       transforms.Pad([8, 8, 8, 8], fill=0, padding_mode='constant'),
                                       transforms.ToTensor(),
                                       transforms.Normalize([0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010])])
    ...
    train_images_3 = ImageFolder(root= "./data/train", transform=train_transform_3)
    test_images_3 = ImageFolder(root= "./data/test", transform=test_transform_3)
    
    # Create data loader
    trainDataLoader_3 = torch.utils.data.DataLoader(train_images_3,batch_size=128,num_workers=2, shuffle=True)
    testDataLoader_3 = torch.utils.data.DataLoader(test_images_3,batch_size=100,num_workers=2, shuffle=False)
    ...
    for epoch in range(50):
        ...
        train_loss, train_accuracy = train(trainDataLoader_3, optimizer, Loss, device, model)                             
        test_loss, test_accuracy = test(testDataLoader_3, Loss, device, model)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("./model_plots/aug3_%s_loss.png" % args.optimizer)
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("./model_plots/aug3_%s_accuracy.png" % args.optimizer)
